...
const slice = createSlice({
  ...
  reducers: {
    ...
    channelDeleted: (guilds, { payload }) => {
      ...
      guild!.channels = guild!.channels.filter(
...
export const fetchMyGuilds =
  () => (dispatch, getState: () => Store.AppStore) => {
...
export const deleteChannel =
  (guildId: string, channelId: string) => (dispatch) => {
    dispatch(
      api.wsCallBegan({
...
export const getGuild = (id: string) =>
  createSelector(
    (state: Store.AppStore) => state.entities.guilds.list,
...
export const getAbbr = (name: string | undefined) =>
  name
    ?.split(" ")
    .map((n) => n[0])
    .join("")
