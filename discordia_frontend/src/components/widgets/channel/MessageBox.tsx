...
const MessageBox: React.FunctionComponent<MessageBoxProps> = (props) => {
  ...
  const handleEscape = (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (props.editingMessageId) dispatch(stoppedEditingMessage());
  };
  ...
  const typingMessage = () => {
    ...
    return typers.length > maxTypers
      ? "Múltiplos usuários estão digitando..."
      : `${typingUsers} está digitando...`;
  };
  ...
  return (
    <div className={`${props.editingMessageId ? "mt-2" : "px-4"}`}>
      <Textarea
        ...
        rows={1}
        placeholder={
          !props.editingMessageId ? `Conversar em #${channel.name}` : ""
        }
