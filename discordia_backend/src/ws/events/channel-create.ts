...
export default class implements WSEvent<"CHANNEL_CREATE"> {
  ...
  public async invoke(
    { io, sessions }: WS,
    client: Socket,
    params: API.WSPayload.ChannelCreate
  ) {
    ...
    for (const client of connectedClients) client.join(channel.id);

    io.to(guild.id).emit("CHANNEL_CREATE", {
      channel,
      creatorId: userId,
    } as API.WSResponse.ChannelCreate);
  }
}
